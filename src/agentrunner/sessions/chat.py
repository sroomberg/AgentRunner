...
def _complete(endpoint: str, model_id: str, messages: list[dict], max_tokens: int) -> str:
    payload = json.dumps(
...
def _store_history(session: Session, user_message: str, response: str) -> None:
    try:
        store = VectorStore(Path(session.db_path))
        embedder = lambda texts: embed(session.endpoint, session.embedding_model, texts)
