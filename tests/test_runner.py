...
from agentrunner.runner import (
    MANAGED_LABEL,
    MODEL_LABEL,
    RunConfig,
    _container_exists,
...
def test_run_config_container_name_default(tmp_path: Path) -> None:
    ...
    assert cfg.container_name == "agentrunner-my-model"
...
def test_parse_labels() -> None:
    labels = _parse_labels("com.agentrunner.managed=true,com.agentrunner.model=llama3")
    assert labels["com.agentrunner.managed"] == "true"
    assert labels["com.agentrunner.model"] == "llama3"


def test_container_exists_false() -> None:
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(returncode=0, stdout="other-container\n")
        assert not _container_exists("agentrunner-llama3")


def test_container_exists_true() -> None:
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(returncode=0, stdout="agentrunner-llama3\n")
        assert _container_exists("agentrunner-llama3")
